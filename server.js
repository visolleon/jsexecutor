...
app.post("/execute", validateToken, (req, res) => {
  ...
  try {
    ...
    if (libraries && Array.isArray(libraries)) {
      console.log(
        `[Task ${taskId}] Validating ${libraries.length} dependencies`,
      );
      ...
      console.log(
        `[Task ${taskId}] ${safeLibraries.length} safe dependencies found`,
      );
    }
    ...
    console.log(
      `[Task ${taskId}] Created package.json with ${safeLibraries.length} dependencies`,
    );
    ...
    try {
      ...
      const stderr = installError.stderr?.toString() || installError.message;
      throw new Error(`npm install failed: ${stderr}`);
    }
    console.log(
      `[Task ${taskId}] Dependencies installed in ${Date.now() - installStart}ms`,
    );
    ...
    const maxMemoryMB = parseInt(MAX_MEMORY);
    try {
      execSync(`node --max-old-space-size=${maxMemoryMB} index.js`, {
        cwd: taskDir,
        stdio: "pipe",
        timeout: MAX_EXECUTION_TIME,
      });
      ...
      throw new Error(`Code execution failed: ${stderr}`);
    }
    console.log(
      `[Task ${taskId}] Code executed in ${Date.now() - execStart}ms`,
    );
    ...
    console.log(
      `[Task ${taskId}] Execution completed in ${totalTime}ms. Generated ${files.length} files.`,
    );
    ...
    console.error(
      `[Task ${taskId || "unknown"}] Error executing code: ${error.message}. Execution time: ${totalTime}ms`,
    );
    ...
    if (taskDir && fs.existsSync(taskDir)) {
      try {
        ...
        console.error(
          `[Task ${taskId || "unknown"}] Error cleaning up task directory: ${cleanupError.message}`,
        );
      }
    }
...
app.listen(port, () => {
  ...
  console.log(
    `Files will be available at: http://localhost:${port}/tasks/{taskId}/{filename}`,
  );
});
